# Awesome-System-Design-Tutorial
Curated list of the best free resources for learning "System Design" (2026 edition). I recently discovered this excellent site that teaches System Design really well. This repo contains all the useful links, roadmaps, courses, and notes I collected from there + other great sources. 

# A lil Introduction to System Design: 
"System Design" is the process of designing the architecture, components, and interfaces for a system so that it meets the end-user requirements. This specifically designed System Design tutorial will help you to learn and master System Design concepts in the most efficient way, from the basics to the advanced level.

## Table of Contents:
- [Basics](#basics)
- [High Level Design](#high-level-design)
- [System Architectural Styles](#system-architectural-styles)
- [Scalability](#scalability)
- [Databases in Designing Systems](#databases-in-designing-systems)
- [Consistency, Availability, Reliability & Maintainability](#consistency-availability-reliability-and-maintainability)
- [Load Balancing](#load-balancing)
- [Latency, Throughput and Caching](#latency-throughput-and-caching)
- [API Gateway, Message Queues & Rate Limiting](#API-gateway-message-queues-and-rate-limiting)
- [Protocols, CDN, Proxies & WebSockets](#Protocols-CDN-Proxies-and-WebSockets)
- [Event-Driven Architecture](#Event-Driven-Architecture)
- [Testing](#Testing)
- [Security Measures](#Security-Measures)
- [Distributed System Design](#Distributed-System-Design)
- [Cost & Performance Optimizations](#Cost-&-Performance-Optimizations)
- [Low Level Design(LLD)](#Low-Level-Design)
- [Design Principles](#Design-Principles)
- [UML](#uml)
- [Design Patterns](#Design-Patterns)
  

## Courses:
- Course 1 Basics →
  [leve1 1.0](https://www.geeksforgeeks.org/system-design/getting-started-with-system-design/)
  [level 1.1 ](https://www.geeksforgeeks.org/software-engineering/functional-vs-non-functional-requirements/)
  [level 1.2](https://www.geeksforgeeks.org/system-design/difference-between-high-level-design-and-low-level-design/)
- Course 2 name → [Link](url)



## How to Use This List:
- ⭐ Star this repo if you find it helpful
- Feel free to fork and add your own resources
- Contributions are welcome!

## Connect with me:
- [Twitter/X](https://x.com/bishop_johnxn)

## Credits:
- geeksforgeeks.org 
. Made with ❤️ for the developer community
